/**
 * Pruebas unitarias para la persistencia y gestion de cola de
 * notificaciones de pago movil pendientes de confirmacion.
 */
...
const sampleNotification2: ParsedBankNotification = {
  ...
  amountCents: 176000,
  currency: 'VES',
  sender: 'JESUS DARIO AZUAJE MANZANILLA',
  reference: '007186969576',
  rawTitle: 'PagomóvilBDV recibido',
  rawBody: 'Recibiste un PagomovilBDV de JESUS DARIO AZUAJE MANZANILLA por Bs.1.760,00',
  detectedAt: '2026-08-29T15:45:00.000Z'
}

describe('Almacenamiento de notificaciones de pago movil', () => {
  beforeEach(async () => {
  ...
  it('devuelve false al fallar la escritura en AsyncStorage', async () => {
    jest.spyOn(AsyncStorage, 'setItem').mockRejectedValueOnce(new Error('almacenamiento lleno'))
  ...
  it('falla en silencio al desencolar si AsyncStorage no responde', async () => {
    await enqueueBankNotification(sampleNotification1)
    jest.spyOn(AsyncStorage, 'setItem').mockRejectedValueOnce(new Error('fallo'))

    await expect(dequeueBankNotification(sampleNotification1.detectedAt)).resolves.toBeUndefined()
  })

  it('limpia todas las notificaciones pendientes', async () => {
    await enqueueBankNotification(sampleNotification1)
    await enqueueBankNotification(sampleNotification2)
  ...
  it('falla en silencio al limpiar si AsyncStorage no responde', async () => {
    jest.spyOn(AsyncStorage, 'removeItem').mockRejectedValueOnce(new Error('fallo'))

    await expect(clearBankNotifications()).resolves.toBeUndefined()
  })
})
