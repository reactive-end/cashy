...
describe('useBankPayments', () => {
  ...
  it('devuelve false si se confirma sin notificacion activa', async () => {
    const { result } = await renderHook(() => useBankPayments())
    await waitFor(() => expect(result.current.activeNotification).toBeNull())

    let ok = true
    await act(async () => {
      ok = await result.current.confirm('Concepto huerfano')
    })
  ...
  it('devuelve false cuando el guardado en base de datos falla', async () => {
    getPendingMock.mockResolvedValue([sampleNotification])

    const { result } = await renderHook(() => useBankPayments())
    await waitFor(() => expect(result.current.activeNotification).not.toBeNull())

    insertIncomeMock.mockRejectedValueOnce(new Error('fallo de base de datos'))

    let ok = true
    await act(async () => {
      ok = await result.current.confirm('Cobro con error')
    })
  ...
  it('ignora el descarto sin notificacion activa', async () => {
    const { result } = await renderHook(() => useBankPayments())
    await waitFor(() => expect(result.current.activeNotification).toBeNull())
