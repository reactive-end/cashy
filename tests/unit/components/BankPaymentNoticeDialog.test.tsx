...
describe('BankPaymentNoticeDialog', () => {
  ...
  it('muestra datos extraidos del pago movil y equivalencia en moneda base', async () => {
    const screen = await render(
      <BankPaymentNoticeDialog
  ...
  it('omite la equivalencia cuando no hay tasas disponibles', async () => {
    const screen = await render(
      <BankPaymentNoticeDialog
        notification={sampleNotification}
        visible={true}
  ...
  it('omite el remitente cuando la notificacion no lo trae', async () => {
    const screen = await render(
      <BankPaymentNoticeDialog
        notification={{ ...sampleNotification, sender: undefined }}
        visible={true}
        rates={mockRates}
        baseCurrency="USD"
        onConfirm={jest.fn()}
  ...
  it('sugiere el concepto inicial con banco y remitente', async () => {
    const screen = await render(
      <BankPaymentNoticeDialog
        notification={sampleNotification}
        visible={true}
    ...
    expect(input.props.value).toBe('Pago Móvil BNC (0414***69)')
  })
